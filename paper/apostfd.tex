...
\begin{Rem} The reader will note that the bound (\ref{eq:bound_ohlberger}) is sub-optimal in the pre-shock and optimal post-shock.  Hence, combining this result with (\ref{eq:burgers_bound}) by choosing the minimum between the two results gives optimal a posteriori error  control both before and after shock formation, albeit in different norms. We demonstrate this combination using an experiment where a the solution loses regularity and eventually a shock develops.
\end{Rem}
