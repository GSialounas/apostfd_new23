...
\begin{abstract}
  ...
  a posteriori estimates for general FD schemes. The results are
  conditional, as they hold under certain conditions on the numerical
...
introduces the spatial and temporal discretizations employed in our
study. WENO schemes, leveraged as spatial discretizations in several
of our examples, are detailed in
\S\ref{sec:WENO_discretization}. Here, we expound upon the underlying
...
context of hyperbolic conservation laws. Section
\S\ref{sec:experiments} is devoted to the presentation and thorough
analysis of our numerical experiments. Our findings and observations,
both anticipated and unforeseen, are meticulously dissected to
validate our theoretical postulations. Moreover, this section offers
insights into the practical implications and potential limitations of
our a posteriori estimates.

The final section, \S\ref{sec:conclusions}, furnishes a reflective conclusion to our study. We encapsulate the primary takeaways, underscore the significance of our contribution to the existing literature, and ruminate on potential directions for future research. Implicitly woven throughout our discourse is the underlying aspiration to bridge the existing chasm between theory and real-world application. By enhancing the understanding of a posteriori error estimates in the context of finite difference schemes for hyperbolic conservation laws, we aim to empower practitioners with tools that are both theoretically rigorous and practically impactful.
...
defined as point values over the space-time domain. This is not a
...
\begin{figure}[H]
	...
	\\
	\begin{subfigure}[b]{\textwidth}
		\includegraphics[width=\textwidth]{../figures/fig_CNCS_plots_1x5_sin_IC_ind_uniform_P1_5555}	
		\caption{\label{sfig:FTCS_prelim_P1}
		  CNCS (\ref{eq:FTCS}).
                  \tristan{loosk at the yaxis on the errors. You need to run the 2nd plot from 1e-6 to 1e0.}
		}
	\end{subfigure}
...
post-processor, effectively augmenting $\widehat U$ such that it is
...
\end{split}
\end{equation}
Note that we have use the sub-script $j$ to denote the spatial position, $x_j$, on the grid where we obtain the $\widehat{U}^t_j$.  Then, we use  $\widehat{U}^t_j$ to compute the full reconstruction $\widehat{ U}^{ts}$, by satisfying two nodal equivalence conditions and a derivative condition to obtain the bi-quadratic $\widehat{ U}^{ts}$ in the patch $\qb{t^n,t^{n+1}}\times\qb{x_j,x_{j+1}}$, by satisfying 
\begin{equation}
 \begin{split}
...
\tristan{I changed this to be CNCS, is it correct? - George: corrected}
...
\begin{Rem}[a posteriori bound for nonlinear problems]
 The test cases we examine in \S \ref{sec:numerical_verification} are nonlinear, so the
...
\begin{Rem} The reader will note that the bound (\ref{eq:bound_ohlberger}) is sub-optimal in the pre-shock but has the advantage that it does not blow-up in the post-shock.  Hence, combining this result with (\ref{eq:burgers_bound}) by choosing the minimum between the two results gives optimal a posteriori error  control both before and after shock formation, albeit in different norms. We demonstrate this combination using an experiment where a the solution loses regularity and eventually a shock develops - see test \S \ref{test:burger_hat}.
\end{Rem}
...
The numerical experiments conducted in this section hitherto highlight the loss of optimality for the bound  (\ref{eq:burgers_bound}) as the solution loses regularity and eventually blows up in the post-shock regime (see Fig. \ref{fig:SSP3WENO_burgers_decoupled}).  We address the issue of post-shock optimality  by utilising an a posteriori estimate from \cite[Thm. 5]{ohlberger2001posteriori}, Thm. \ref{thm:aposteriori_ohlberger}, which converges as $\mathcal{O}\qp{h^{1/2}}$ throughout the computation.  
...
\end{cases}
\end{equation}
The results are shown in Fig. \ref{fig:FTBS_burger_step}.  We adopt the following notation for ease of reference:
\begin{equation}
\begin{aligned}
\w_b\mathcal{E}_b^2
&:=
