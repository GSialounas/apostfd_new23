...
q = 32*pi;%100; % Sets the upper domain boundary
L_domain = q;
h_l = .2;
h_r = .1;
...
%%george's stuff for the exact solution
fps =10;
g=9.81;
d= 2;
lambda = 32*pi;
...
s = 0.1; % Safety Factor for timestep calculation
runningtime = 200;  % Set time simulation runs for
...
while t < runningtime  % Halts the program when condition is violated
    ...
    if mod(rep-1,fps) ==0
        ...
        subplot(2,1,1)
        plot(x, h(2:N+1),'b', x, ex_soln_h, 'r')  % Plots water height aproximations for each timestep.
        ...
        subplot(2,1,2)
        plot(x, v(2:N+1).*h(2:N+1),'b', x, ex_soln_hv, 'r')   % Plots water velocity aproximations for each timestep.
        axis([p q min(v) 1.1*max(h.*v)])
