...
rep_video=1;
mesh_adapt=1;
mesh_comp = 'equivalent';
if mesh_adapt == 1
    adaptivity = 'adapt_ON';
else
    if mesh_comp == "equivalent"
        adaptivity = 'adapt_OFF';
    elseif mesh_comp == "fine"
        adaptivity = 'adapt_OFF_fine';
    elseif mesh_comp == "coarse"
        adaptivity = 'adapt_OFF_coarse';
    else
    end
...
scheme_string= ['SHW_dambreak_',time_stepping,'_',spat_disc,'_',rec,'_',dt_dx_coupling,'_gs'];
...
% fn_spat_disc = @(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain) WENO3_shw_dambreak_nonu_frompaper(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);

if mesh_adapt ==0 
%     fn_spat_disc = @(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain) WENO3resAdv1d_FD_system_just_for_rec_dambreak(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
    fn_spat_disc = @(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain) WENO3_shw_dambreak_nonu_naive(un_arr,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
...
for m = 1:1
    ...
    it =0;
    grid_power = 9; % usually this is 8
    dx = L_domain*(2^(-(maxit_arr(m)+grid_power))); % mesh size
...
%         M.refineGlobal();
        ...
        dx = x(2) -x(1);
        dt = .5*.5*.1*(x(2)-x(1)); % four ref levels
        T =2*2^(grid_power-8)*2000*dt; % 2000 dt corresponds to 10 seconds run time 
                          %when dx = L_domain*(2^(-(maxit_arr(m)+8))); 
                          % Successive increments of 2 are needed after
                          % that
        if mesh_comp == "fine"
        len_x_original = length(x);
...
%     mesh_adapt= 0;
    if mesh_adapt == 0
       ...
       x = linspace(0,L_domain,len_x_original);
       dx= x(2)-x(1);
       dist_x_pl = x(2:end) - x(1:end-1);
       h = fn_hinit_dambreak(x);
       v = zeros(size(h));
...
%         [~,f_h_old_weno] = WENO3_shw_dambreak_nonu([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
        [~,f_h_old_weno] = fn_spat_disc([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
...
%         [~,f_h_stage1_weno] = WENO3_shw_dambreak_nonu([un1_stage1;un2_stage1],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1_stage1,F2_stage1,x,t,L_domain);
        [~,f_h_stage1_weno] =fn_spat_disc([un1_stage1;un2_stage1],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1_stage1,F2_stage1,x,t,L_domain);
...
%         [~,f_h_stage2_weno] = WENO3_shw_dambreak_nonu([un1_stage2;un2_stage2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1_stage2,F2_stage2,x,t,L_domain);
        [~,f_h_stage2_weno] = fn_spat_disc([un1_stage2;un2_stage2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1_stage2,F2_stage2,x,t,L_domain);
...
%         [poly_rec2, poly_rec_x2] = fn_WENO3_rec_nonu_dambreak(x, x,dist_x_pl, [h;h.*v],L_domain);
        [poly_rec2, poly_rec_x2] = fn_WENO3_rec_nonu_dambreak(x, x,dist_x_pl, [ex_soln_h;ex_soln_hv],L_domain);

%         f_h_new_weno = WENO3resAdv1d_FD_system_just_for_rec_dambreak([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t);
%          [poly_rec,f_h_new_weno] = WENO3_shw_dambreak_nonu([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
%          [poly_rec,f_h_new_weno] = WENO3_shw_dambreak_nonu([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
         [poly_rec,f_h_new_weno] = fn_spat_disc([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
...
%         end
        ...
        if mesh_adapt==0
            ...
            for iq = 1 : nq
                ...
                if rec =="rec_1"
                    [RL2iq,RL2iq_h,RL2iq_hv,RL2iq_arr, c_0_coeff_arr_new, c_0_coeff_arr_old,IU_h, IU_hv, R_h, R_hv,IUx_h, IUx_hv, IUt_h, IUt_hv,RL2iq_h_arr,RL2iq_hv_arr, max_IUx] = compute_Rs_vector_temp_1_spatiotemp_1_shw_dambreak(x,dist_x_pl,dist_x_min,h_old,h_new,tq(iq),t,dt,f_h_old_weno,f_h_new_weno,'CS',flux_fn); %compute R at gauss point
                elseif rec =="rec_3"
                    ...
                    [RL2iq,RL2iq_h,RL2iq_hv,RL2iq_arr, c_0_coeff_arr_new, c_0_coeff_arr_old,IU_h, IU_hv, IUt_h, IUx_hv,IUt_hv, IUx_hv_flux , max_IUx] = compute_Rs_vector_temp_3_spatiotemp_3_shw_weno3_dambreak(x,dist_x_pl,dist_x_min,h_old,h_new,tq(iq),t,dt,f_h_old_weno,f_h_new_weno,'CS',flux_fn,flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2); %compute R at gauss point
%                     [L2Rt,L2Rt_h,L2Rt_hv,L2Rt_arr,c_0_coeff_arr_new, c_0_coeff_arr_old,IU_h, IU_hv, R_h, R_hv,IUx_h, IUx_hv,IUt_h, IUt_hv,L2Rt_h_arr,L2Rt_hv_arr, max_IUx]  
                else
                end
...
%                     [RL2iq,RL2iq_h,RL2iq_hv,RL2iq_arr, c_0_coeff_arr_new, c_0_coeff_arr_old,IU_h, IU_hv, R_h, R_hv,IUx_h, IUx_hv, IUt_h, IUt_hv,RL2iq_h_arr,RL2iq_hv_arr] = compute_Rs_vector_temp_1_spatiotemp_1_shw_dambreak(x,dist_x_pl,dist_x_min,h_old,h_new,tq(iq),t,dt,f_h_old,f_h_new,'CS',flux_fn); %compute R at gauss point
            ...
            if it ==0 && mesh_adapt==1
                ...
                error_0 = space_int_vector_rec_weno_nonu_dambreak(x,dist_x_pl,dist_x_min,h_old,0*dt,fn_exact_soln,L_domain);
            end
            h = h_old(1,:);
            v= h_old(2,:)./h_old(1,:);
...
%             f_h_new_weno = WENO3resAdv1d_FD_system_just_for_rec_dambreak([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t);
            [~,f_h_new_weno] = fn_spat_disc([un1;un2],flux,dflux,S,dx,eig_Jf_1, eig_Jf_2,F1,F2,x,t,L_domain);
            [ex_soln_h, ex_soln_hv] = fn_exact_soln(x,t);
            [poly_rec2, poly_rec_x2] = fn_WENO3_rec_nonu_dambreak(x, x,dist_x_pl, [ex_soln_h;ex_soln_hv],L_domain);
...
%         t = t + dt  ;
        ...
        if (mod(it-1,fps)==0 &&showplots==1)
            %             S_forcing = S(x,t);
%             colororder({'b',11*[0.05 0.05 0.05]})
...
%             plot(x,h,'b',x, ex_soln_h,'ro')
            ...
            h2=  semilogy(x(1:end-1), 1./dist_x_pl,'LineWidth',1,'Color',13*[0.05 0.05 0.05]);
            ylim([1e+0 1e+3])
            set(gca,'ycolor',9*[0.05 0.05 0.05]) 
            %
            % %             plot(x,h-ex_soln_h,'r')
            % hold on;
            ylabel('$\ln\left(1/{h_j}\right)$','Interpreter','latex','FontSize',f_size)
            xlabel('$x_j$','Interpreter','latex','FontSize',f_size)
            grid off;
            
            set(gca,'Color','None')
              yyaxis right
               h1 = plot(x,h,'LineWidth',1,'Color',[0 0.4470 0.7410]);
...
%             uistack(h2,'top')
%             if (it ==5001)
               % legend({'$u$','$\ln\left(1/{h_j}\right)$'},'Interpreter','latex','Location',[0.39 0.075 0.2 0.06],'FontSize',15);
...
%                 saveas(figgy_parasite,['/Users/gs1511/Desktop/GSialounas_BBucket_ApostFD_repo/apostfd/figures/fig_shw_dambreak',scheme_arr{i_scheme},'_stills_',num2str(it)],'png')
                print(gcf,['/Users/gs1511/Desktop/GSialounas_BBucketnew_apostfd/apostfd/figures/fig_shw_dambreak',scheme_arr{i_scheme},'_stills_',num2str(it),'_adaptONOFF'],'-dpng','-r600')
...
%         
        ...
        if it == 0 
            ...
            %                 bound_arr(end+1) = sqrt(exp(1*dt)*(L2L2R + space_int_vector_gs(x_0,dist_x_pl_0,dist_x_min_0,ex(x_0,0),0,ex,c_0_coeff_arr_0)));
            error_new = sqrt( space_int_vector_rec_weno_nonu_dambreak(x,dist_x_pl,dist_x_min,[h; h.*v],1*dt,fn_exact_soln,L_domain));
            %                 error_new = sqrt(space_int_vector_gs(x,dist_x_pl,dist_x_min,u,1*dt,ex,c_0_coeff_arr_new));
            error_arr(end+1) =error_new;% max(error_new, error_old);
            error_old = max(error_arr);
            EI_index(end+1) = bound_arr(end)./error_arr(end);
...
%             error_new = sqrt(space_int_vector_gs(x,dist_x_pl,dist_x_min,u,(it+1)*dt,ex,c_0_coeff_arr_new));
            error_new = sqrt( space_int_vector_rec_weno_nonu_dambreak(x,dist_x_pl,dist_x_min,[h; h.*v],(it+1)*dt,fn_exact_soln,L_domain));
    ...
    if (save_results_to_cellarr==1)
        cell_cell_arr_shw{m}=[time_arr;bound_arr;error_arr;EI_index;dofs_arr];
    end
end
