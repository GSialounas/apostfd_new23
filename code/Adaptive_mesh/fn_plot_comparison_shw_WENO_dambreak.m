function var_out = fn_plot_comparison_shw_WENO_dambreak(u_init,v_init, ylim_plot1, ylim_plot2, xlim_plot, EI_plot_y_lim,R_plot_lims,pow,scheme_name_str, init_conds_str, interpolant_str)
...
for i = 1 : length(scheme_arr)
    ...
    c_OFF_fine{i}=struct2cell(load([scheme_arr{i},'_cell_arr_file_shw_adapt_OFF_fine.mat']));
    c_OFF_coarse{i}=struct2cell(load([scheme_arr{i},'_cell_arr_file_shw_adapt_OFF_coarse.mat']));
...
colour_arr_OFF_fine =['y';'c';'g';"mo";"ro";"b--o";"k."];
colour_arr_OFF_coarse =['y';'c';'g';"mo";"ro";"b--o";"k--"];
...
%         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        ...
        time_arr_OFF_fine  =  c_OFF_fine{i_scheme}{1}{i_ref}(1,:);
        bound_arr_OFF_fine  =  c_OFF_fine {i_scheme}{1}{i_ref}(2,:);
        error_arr_OFF_fine  =  c_OFF_fine {i_scheme}{1}{i_ref}(3,:);
        %         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        dofs_arr_OFF_fine   =  c_OFF_fine {i_scheme}{1}{i_ref}(5,:);
        
        time_arr_OFF_coarse  =  c_OFF_coarse{i_scheme}{1}{i_ref}(1,:);
        bound_arr_OFF_coarse =  c_OFF_coarse{i_scheme}{1}{i_ref}(2,:);
        error_arr_OFF_coarse =  c_OFF_coarse{i_scheme}{1}{i_ref}(3,:);
%         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        dofs_arr_OFF_coarse  =  c_OFF_coarse{i_scheme}{1}{i_ref}(5,:);
        
        
        % get the L_inf(L2 norm of errors)
        max_ON=error_arr_ON(1);
        max_OFF= error_arr_OFF(1);
        max_OFF_coarse=error_arr_OFF_coarse(1);
        max_OFF_fine = error_arr_OFF_fine(1);
        ...
        for i = 1: length(time_arr_ON)
           ...
           error_arr_OFF_fine_max(i) = max(error_arr_OFF_fine(1:i));
           error_arr_OFF_coarse_max(i) = max(error_arr_OFF_coarse(1:i));

            
%             if error_arr_OFF(i)>max_OFF
%                 error_arr_OFF(i) = max_OFF;
%             else
%                 max_OFF= error_arr_OFF(i);
%             end
%             
%             if error_arr_OFF_fine(i)>max_OFF_fine
%                 error_arr_OFF_fine(i) = max_OFF_fine;
%             else
%                 max_OFF_fine= error_arr_OFF_fine(i);
%             end
%             
%             if error_arr_OFF_coarse(i)>max_OFF_coarse
%                 error_arr_OFF_coarse(i) = max_OFF_coarse;
%             else
%                 max_OFF_coarse= error_arr_OFF_coarse(i);
%             end
        end
        error_arr_ON = error_arr_ON_max;
        error_arr_OFF = error_arr_OFF_max;
        error_arr_OFF_fine = error_arr_OFF_fine_max;
        error_arr_OFF_coarse = error_arr_OFF_coarse_max;
...
%         
        ...
        subplot(1,N_subplots,1)
        semilogy(time_arr_ON,dofs_arr_ON,colour_arr(end-(l_refs-i_ref)),time_arr_OFF,dofs_arr_OFF,[colour_arr_OFF(end-(l_refs-i_ref))],time_arr_OFF_fine,dofs_arr_OFF_fine,'b',time_arr_OFF_coarse,dofs_arr_OFF_coarse,'g','LineWidth',2)
        ...
        hold on;
         ...
         subplot(1,N_subplots,2)
        semilogy(cumsum(dofs_arr_ON),error_arr_ON_max,colour_arr(end-(l_refs-i_ref)),cumsum(dofs_arr_OFF),error_arr_OFF_max,[colour_arr_OFF(end-(l_refs-i_ref))],cumsum(dofs_arr_OFF_fine),error_arr_OFF_fine_max,'b',cumsum(dofs_arr_OFF_coarse),error_arr_OFF_coarse_max,'g','LineWidth',2)
        %         ylabel(['$ \left|\left|\mathcal{E}\left(t_i\right)\right|\right|_{L_2\left(0,t_i\right)} $'],'Interpreter','latex')
        %         title(['$\mathcal{P}^',num2str(2),'$',': $dt=0.1dx^', num2str(1),'$'],'Interpreter','latex')
...
%         ylim([0,1100])
        hold on;
        legend('adaptive','uniform-equivalent','uniform-finest','uniform-coarsest','Location','SouthEast')
...
%         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        ...
        time_arr_OFF_fine  = time_arr_OFF_fine(1:40:end);
        bound_arr_OFF_fine  = bound_arr_OFF_fine(1:40:end);
        error_arr_OFF_fine  =  error_arr_OFF_fine(1:40:end);
        %         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        dofs_arr_OFF_fine   =  dofs_arr_OFF_fine(1:40:end);
        
        time_arr_OFF_coarse  = time_arr_OFF_coarse(1:40:end);
        bound_arr_OFF_coarse =  bound_arr_OFF_coarse(1:40:end);
        error_arr_OFF_coarse = error_arr_OFF_coarse(1:40:end);
%         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        dofs_arr_OFF_coarse  = dofs_arr_OFF_coarse(1:40:end);
...
%          print(gcf,['/Users/gs1511/Desktop/GSialounas_repo_temporal_rec/fig_',scheme_arr{i_scheme},'plots_1x5_sin_IC_harmonic_u',num2str(round(10*u_init)),'_v',num2str(round(10*v_init)),'_paperrec_poly_tristan.png'],'-dpng','-r600')
         print(gcf,['/Users/gs1511/Desktop/GSialounas_BBucket_ApostFD_repo/apostfd/figures/fig_',scheme_arr{i_scheme},'plots_1x5_shw_dambreak_comparison_adaptONOFF'],'-dpng','-r600')
        
         arr_wr_names = ["time_arr_ON_shw_dambreak", "error_arr_ON_shw_dambreak", "error_arr_OFF_shw_dambreak","error_arr_OFF_shw_dambreak_fine","error_arr_OFF_shw_dambreak_coarse"];
         arr_wr = [time_arr_ON', error_arr_ON', error_arr_OFF',error_arr_OFF_fine',error_arr_OFF_coarse'];
         fid  = fopen('../../paper/data/error_file_adapt_shw_dambreak.csv','w');
         fprintf(fid,'%s, %s, %s, %s, %s\n',arr_wr_names(1), arr_wr_names(2), arr_wr_names(3) ,arr_wr_names(4), arr_wr_names(5))
         ...
         arr_wr_names = ["time_arr_ON_shw_dambreak", "dofs_arr_ON_shw_dambreak", "dofs_arr_OFF_shw_dambreak","dofs_arr_OFF_shw_dambreak_fine","dofs_arr_OFF_shw_dambreak_coarse"];
         arr_wr = [time_arr_ON', dofs_arr_ON', dofs_arr_OFF', dofs_arr_OFF_fine', dofs_arr_OFF_coarse'];
         fid  = fopen('../../paper/data/dofs_file_adapt_shw_dambreak.csv','w');
         fprintf(fid,'%s, %s, %s, %s, %s\n',arr_wr_names(1), arr_wr_names(2), arr_wr_names(3), arr_wr_names(4), arr_wr_names(5))
