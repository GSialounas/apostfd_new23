function A = createA_advection_TRISTAN(U, dt, x,Lx)
...
% x_d       =     zeros(N,1); % Main Diagonal Entries
% x_dplus1  =     U * dt * [0 ; (x(2)-x(1) + (Lx-x(end))).^(-1) ; (x(3:end)-x(1:(end-2))).^(-1)]; % First superdiagonal
% % x_dplus1  =     U * dt * [0 ; (x(2)-x(1) + (x(end)-x(end-1))).^(-1) ; (x(3:end)-x(1:(end-2))).^(-1)]; % First superdiagonal
% x_dminus1 =     - U * dt * [(x(3:end)-x(1:(end-2))).^(-1) ; (x(2)-x(1) + (Lx-x(end)))^(-1) ; 0]; % First subdiagonal
...
% f(1)      =     U * dt * (x(2)-x(1) + (Lx-x(end)))^(-1); % penultimate subdiagonal
% % f(1)      =     U * dt * (x(2)-x(1) + (x(end)-x(end-1)))^(-1); % penultimate subdiagonal
% f(end)  =     -U * dt *  (x(2)-x(1) + (Lx-x(end)))^(-1); % penultimate superdiagonal
...
x_dplus1  =     U * dt * [0 ; (x(2)-x(1) + (Lx-x(end))).^(-1) ; (x(3:end)-x(1:(end-2))).^(-1)]; % First superdiagonal
% x_dplus1  =     U * dt * [0 ; (x(2)-x(1) + (x(end)-x(end-1))).^(-1) ; (x(3:end)-x(1:(end-2))).^(-1)]; % First superdiagonal
x_dminus1 =     - U * dt * [(x(3:end)-x(1:(end-2))).^(-1) ; (Lx-x(end-1))^(-1) ; 0]; % First subdiagonal
% x_dminus1 =     - U * dt * [(x(3:end)-x(1:(end-2))).^(-1) ; (x(2)-x(1) + (x(end)-x(end-1)))^(-1) ; 0]; % First subdiagonal
f         =     zeros(size(x_d)); % penultimate sup and sub diagonals
f(1)      =     U * dt * (Lx-x(end-1))^(-1); % penultimate subdiagonal
% f(1)      =     U * dt * (x(2)-x(1) + (x(end)-x(end-1)))^(-1); % penultimate subdiagonal
f(end)  =     -U * dt *  (x(2)-x(1) + (Lx-x(end)))^(-1); % penultimate superdiagonal
% f(end)  =     -U * dt *  (x(2)-x(1) + (x(end)-x(end-1)))^(-1); % penultimate superdiagonal
