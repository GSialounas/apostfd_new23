...
for i_scheme = 1:length(scheme_arr)
    ...
    for m =length(maxit_arr):length(maxit_arr)
        ...
        dt =  .1 * 2^(-(maxit_arr(end)+6));%.1*h^1; % initial time-step
        dx_arr(m) = h;
...
%             M.refineGlobal();
        ...
        while t <= T-dt/2          
            ...
            uold_x = 1./(dist_x_min).*(circshift(uold,0) - circshift(uold,1) );
            u= uold - dt*uold_x;
            u_x = 1./(dist_x_min).*(circshift(u,0) - circshift(u,1));
...
%                     L2L2R = L2L2R + wq(iq)*dt*(RL2iq); %quadrature formula
                ...
                uold_x = 1./(dist_x_min).*(circshift(uold,0) - circshift(uold,1) );
                u= uold - dt*uold_x;
                u_x = 1./(dist_x_pl) .* (u-circshift(u,1));
        ...
        if (record_video)
            ...
            video = VideoWriter(['/Users/gs1511/Desktop/GSialounas_BBucket_ApostFD_repo/apostfd/videos/FTBS_adapt_linadvect_step_ic','_',num2str(maxit_arr(m)),'_solution_with_',adaptivity,'.mp4'],'MPEG-4');
            video.FrameRate = fps;
            open(video);
