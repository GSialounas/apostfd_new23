function var_out = fn_plot_comparison_lin_advect(u_init,v_init, ylim_plot1, ylim_plot2, xlim_plot, EI_plot_y_lim,R_plot_lims,pow,scheme_name_str, init_conds_str, interpolant_str)
...
%         EI_arr    =  c_OFF{i_scheme}{1}{i_ref}(4,:);
        ...
        for i = 1:length(error_arr_ON)
            if i>1
            error_arr_ON(i) = max(error_arr_ON(i), error_arr_ON(i-1));
             error_arr_OFF(i) = max(error_arr_OFF(i), error_arr_OFF(i-1));
            end
        end
...
%         ylim([0,1100])
        ...
        time_arr_ON  =  time_arr_ON(1:50:end);
        bound_arr_ON =  bound_arr_ON(1:50:end);
        error_arr_ON =  error_arr_ON(1:50:end);
%         EI_arr    =  c_ON{i_scheme}{1}{i_ref}(4,:);
        dofs_arr_ON  =  dofs_arr_ON(1:50:end);
        
        time_arr_OFF  =  time_arr_OFF(1:50:end);
        bound_arr_OFF =  bound_arr_OFF(1:50:end);
        error_arr_OFF =  error_arr_OFF(1:50:end);
        dofs_arr_OFF  = dofs_arr_OFF(1:50:end);
%          subplot(1,3,3)
%         %         title(['$dt=0.1dx^', num2str(exponent_arr(1)),'$'],'Interpreter','latex')
