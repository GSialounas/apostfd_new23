...
%     IUt = fn_WENO3_rec_burger(xiq, x,dist_x_pl, Ut_t);
    ...
    integral_txq = (wq(iq)*dist_x_pl.*1.*( (IUt+IU.*IUx)).^2);
    integral_txq_l1 = (wq(iq)*dist_x_pl.*1.*( abs(IUt+IU.*IUx)));
...
%     integral_txq_l1 = (wq(iq)*dist_x_pl.*( abs(IUt+fUx)));
    ...
    L2Rt = L2Rt + sum(1.*integral_txq);
    L2Rt_arr = L2Rt_arr + 1.*(integral_txq);
    
    L2Rt_l1 = L2Rt_l1 + sum(1.*integral_txq_l1);
    L2Rt_arr_l1 = L2Rt_arr_l1 + (1.*integral_txq_l1);
