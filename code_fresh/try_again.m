...
scheme = 'FTBS';%'FT2S' order 1 time, order 2 space upwinding
...
?
    ...
    i = 0; 
    L2L1R = 0; %L2L1 accumulation of ||R||
    ...
    while t < T+eps
        ...
        for iq = 1 : nq
            ...
            L2L1R = L2L1R + wq(iq)*dt*(RL1iq); %quadrature formula
        end
        t = t + dt; %move in time
        uold = u; 
?
    end
    [L1, L2] = space_int(x,u,T,ex);
    finalL1err(m) = abs(L1); %compute final time L2 error
    finalL2err(m) = sqrt(L2); %compute final time L2 error
    RL2(m) = sqrt((space_int(x,ex(x,0),0,ex) + L2L2R)); %bound
    RL1(m) = abs((space_int(x,ex(x,0),0,ex) + L2L1R)); %bound
    EOCeL1(1) = 0;
    EOCeL2(1) = 0;
    EOCRL1(1) = 0;
    EOCRL2(1) = 0;
    if m > 1
        EOCeL1(m) = log(finalL1err(m-1)/finalL1err(m))/log(2);
        EOCeL2(m) = log(finalL2err(m-1)/finalL2err(m))/log(2);
        EOCRL2(m) = log(RL2(m-1)/RL2(m))/log(2);
        EOCRL1(m) = log(RL1(m-1)/RL1(m))/log(2);
    end
    EI(m) = RL1(m)/finalL2err(m);
    %fprintf(1,'||(u - IU)(T)||_L1 = %.5f  EOC = %1.2f\n',finalL2err(m),EOCeL1(m))
    %fprintf(1,'||(u - IU)(T)||_L2 = %.5f  EOC = %1.2f\n',finalL2err(m),EOCeL2(m))
